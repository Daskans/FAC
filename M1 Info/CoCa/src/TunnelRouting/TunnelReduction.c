 ...
 * @param stack_size the max size of the height
...
{
    bool enable_existence   = 1;
    bool enable_unicity     = 1;
    ...
    for (int pos = 0; pos <= length; pos ++) {
        ...
        Z3_ast conditions[2];
        if (enable_existence) {
            conditions[n++] = existance_of_node_at_pos(ctx, num_node, stack_size, pos);
                    ...
                    if (u_node != v_node) {
                        ...
                        for (int ku = 0, iter2 = 0; ku <= stack_size; ku++) {
                            ...
                            for (int kv = 0; kv <= stack_size; kv++) {
                                ...
                                all_different_height[iter2] = Z3_mk_or(ctx, 2, unicity);
        ...
        conditions[n++] = Z3_mk_and(ctx, num_node * (num_node - 1), all_different_node);
        // printf("        unicity_of_node_at_pos : conditions[1]\n");
        }
        all_pos[pos] = Z3_mk_and(ctx, n, conditions);
...
Z3_ast initial_node_conditions(Z3_context ctx, const TunnelNetwork network)
{
    ...
    if (tn_node_has_action(network, 0, transmit_4)) {
        formula[2] = Z3_mk_true(ctx);
    } else {
        formula[2] = Z3_mk_false(ctx);
    }
 ...
 * @return Z3_ast
 */
Z3_ast final_node_conditions(Z3_context ctx, const TunnelNetwork network, int length)
{
    Z3_ast formula[2];
    int final_node = tn_get_final(network);
    // printf("    final_node_conditions : final_node = %d\n", final_node);
    formula[0] = tn_path_variable(ctx, final_node, length, 0); // hl = 0
    // printf("    final_node_conditions : formula[0] = %d\n", Z3_get_bool_value(ctx, formula[0]));
    formula[1] = tn_4_variable(ctx, length, 0);                // Hl(hl) = 4
    // printf("    final_node_conditions : formula[1] = %d\n", Z3_get_bool_value(ctx, formula[1]));
    return Z3_mk_and(ctx, 2, formula);
}

/**
 * @brief the formula stating the recieved transmit operation is valid
 *
 * @param ctx the solver context
 * @param network the network
...
Z3_ast transmit_is_correct(Z3_context ctx, TunnelNetwork network, stack_action operation, int length, int pos) {
    ...
    int stack_size = get_stack_size(length);
    Z3_ast all_nodes[num_node * num_node];
    int iter = 0;
        ...
        if (tn_node_has_action(network, u_node, operation)) {
            for (int v_node = 0; v_node < num_node; v_node++) { // sender
                 if (tn_is_edge(network, v_node, u_node)) {
                     ...
                     Z3_ast all_heights[stack_size + 1];
                     for (int k = 0; k <= stack_size; k++) {
     
                         Z3_ast transmit[3];
                         transmit[0] = tn_path_variable(ctx, u_node, pos, k);
                         transmit[1] = tn_path_variable(ctx, v_node, pos - 1, k);
                         if (operation == transmit_4) {
                            transmit[2] = tn_4_variable(ctx, pos - 1, k);
                         } else {
                             transmit[2] = tn_6_variable(ctx, pos - 1, k);
                         }
     
                         all_heights[k] = Z3_mk_and(ctx, 3, transmit);
                     }
                     all_nodes[iter++] = Z3_mk_or(ctx, stack_size + 1, all_heights);
                 }
            }
        }
    }
    return Z3_mk_or(ctx, iter, all_nodes);
}

/**
 * @brief the formula stating the recieved push operation is valid
 *
 * @param ctx the solver context
 * @param network the network
 * @param operation the operation
 ...
 * @return Z3_ast
 */
Z3_ast push_is_correct(Z3_context ctx, TunnelNetwork network, stack_action operation, int length, int pos) {
    int num_node = tn_get_num_nodes(network);
    int stack_size = get_stack_size(length);
    Z3_ast all_nodes[num_node * num_node];
    int iter = 0;
        ...
        if (tn_node_has_action(network, u_node, operation)) {
            for (int v_node = 0; v_node < num_node; v_node++) { // sender
                 if (tn_is_edge(network, v_node, u_node)) {
                     ...
                     Z3_ast all_heights[stack_size];
                     for (int k = 0; k < stack_size; k++) {
     
                         Z3_ast push[4];
                         push[0] = tn_path_variable(ctx, u_node, pos, k);
                         push[1] = tn_path_variable(ctx, v_node, pos - 1, k + 1);
                         if (operation == push_4_4 ||operation == push_4_6) {
                             push[2] = tn_4_variable(ctx, pos - 1, k);
                         } else {
                             push[2] = tn_6_variable(ctx, pos - 1, k);
                         }
                         if (operation == push_4_4 ||operation == push_6_4) {
                            push[3] = tn_4_variable(ctx, pos, k);
                         } else {
                            push[3] = tn_6_variable(ctx, pos, k);
                         }

                         all_heights[k] = Z3_mk_and(ctx, 4, push);
                     }
                     all_nodes[iter++] = Z3_mk_or(ctx, stack_size, all_heights);
                 }
            }
        }
    }
...
/**
 * @brief the formula stating the recieved pop operation is valid
 *
 * @param ctx the solver context
 * @param network the network
 * @param operation the operation
 ...
 * @return Z3_ast
 */
Z3_ast pop_is_correct(Z3_context ctx, TunnelNetwork network, stack_action operation, int length, int pos) {
    int num_node = tn_get_num_nodes(network);
    int stack_size = get_stack_size(length);
    Z3_ast all_nodes[num_node * num_node];
    int iter = 0;
        ...
        if (tn_node_has_action(network, u_node, operation)) {
            for (int v_node = 0; v_node < num_node; v_node++) { // sender
                if (tn_is_edge(network, v_node, u_node)) {
                    ...
                    Z3_ast all_heights[stack_size];
                    for (int k = 1; k <= stack_size; k++) {
     
                        Z3_ast pop[4];
                        pop[0] = tn_path_variable(ctx, u_node, pos, k);
                        pop[1] = tn_path_variable(ctx, v_node, pos - 1, k - 1);
                        if (operation == pop_4_4 ||operation == pop_6_4) {
                            pop[2] = tn_4_variable(ctx, pos - 1, k);
                        } else {
                            pop[2] = tn_6_variable(ctx, pos - 1, k);
                        }
                        if (operation == pop_4_4 ||operation == push_4_6) {
                            pop[3] = tn_4_variable(ctx, pos - 1, k - 1);
                        } else {
                            pop[3] = tn_6_variable(ctx, pos - 1, k - 1);
                        }

                        all_heights[k - 1] = Z3_mk_and(ctx, 4, pop);
                    }
                    all_nodes[iter++] = Z3_mk_or(ctx, stack_size, all_heights);
                }
            }
...
/**
 * @brief the formula stating the recieved operation is valid
 *
 * @param ctx the solver context
 * @param network the network
...
Z3_ast operation_is_correct(Z3_context ctx, TunnelNetwork network, stack_action operation, int length, int pos)
{
    if (operation <= transmit_6) {
        return transmit_is_correct(ctx, network, operation, length, pos);
    } else if (operation <= push_6_6) {
        return push_is_correct(ctx, network, operation, length, pos);
    } else {
        return pop_is_correct(ctx, network, operation, length, pos);
    }
}
...
Z3_ast is_a_path(Z3_context ctx, TunnelNetwork network, int pos, int length)
{
    int stack_size = get_stack_size(length);
    int num_node = tn_get_num_nodes(network);
    // printf("HHHEEEEELLLOOOOOOO : %d, %d\n", num_node * num_node, stack_size * stack_size);
    ...
    for (int u_node = 0, iter = 0; u_node < num_node; u_node++)
    {
        ...
        for (int v_node = 0; v_node < num_node; v_node++)
        {
            ...
            if (tn_is_edge(network, v_node, u_node))
            { // if (v,u)eE then good
                every_edge[iter++] = Z3_mk_true(ctx);
            ...
            { // else it means that either u or v is not good
                ...
                { // 0 <= k <= hmax
                    ...
                    for (int kv = 0; kv < stack_size; kv++) { // 0 <= k' <= hmax
                        ...
                        every_height[iter2++] = Z3_mk_or(ctx, 2, edge_check);

                        // printf("                        is_a_path : every_height[%d]\n", iter2);
                    }
                }
                // printf("                is_a_path : every_edge[%d]\n", iter);
                every_edge[iter++] = Z3_mk_and(ctx, stack_size * stack_size, every_height);
            }
        }
    }
...
{
    bool enable_path            = 1;
    bool enable_height_unicity  = 1;
    bool enable_pos_unicity     = 1;
    int n = 0;
    ...
    if (enable_path) {
        ...
        conditions[n++] = Z3_mk_and(ctx, length, path);
        // printf("    path_conditions : conditions[0] = %d\n", Z3_get_bool_value(ctx, conditions[0]));
    }
    if (enable_height_unicity) {
        conditions[n++] = unicity_of_height_and_simple_path(ctx, num_node, length);
        // printf("    path_conditions : conditions[1] = %d\n", Z3_get_bool_value(ctx, conditions[1]));
    }
    if (enable_pos_unicity) {
        conditions[n++] = unicity_of_node_at_pos(ctx, num_node, length);
        // printf("    path_conditions : conditions[2] = %d\n", Z3_get_bool_value(ctx, conditions[2]));
    }
    return Z3_mk_and(ctx, n, conditions);
}
...
Z3_ast operations_conditions(Z3_context ctx, TunnelNetwork network, int length)
{
    ...
    for (int pos = 1; pos < length; pos++)
    {
        ...
        conditions[pos - 1] = Z3_mk_or(ctx, 10, all_operation);
        // printf("    operations_conditions : conditions[%d]\n", pos);
    }
    return Z3_mk_and(ctx, length - 1, conditions);
}
...
Z3_ast tn_reduction(Z3_context ctx, const TunnelNetwork network, int length)
{
    ...
    Z3_ast conditions[4];
    if (enable_path) {
        conditions[n++] = path_conditions(ctx, network, length);
        // printf("tn_reduction : conditions[0]\n");
    }
    if (enable_initial) {
        conditions[n++] = initial_node_conditions(ctx, network);
        // printf("tn_reduction : conditions[1]\n");
    }
    if (enable_final) {
        conditions[n++] = final_node_conditions(ctx, network, length);
        // printf("tn_reduction : conditions[2]\n");
    }
    if (enable_operations) {
        conditions[n++] = operations_conditions(ctx, network, length);
