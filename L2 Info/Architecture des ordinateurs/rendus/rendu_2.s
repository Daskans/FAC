...
carloop:                        # le calcul ic est (x+1)² = x²+2x+1 ou (%edx+1)² = %edx²+2*%edx+1
        ...
        iaddl   1,%edx          # on termine le calcule avec "+1"
        ...
        iaddl   1,%ecx          # on incrémente x
        jmp     carloop         # on test le carré du nouveau x
        
stopTooBig:
        isubl   1,%ecx          # si le carré de (x+1) est plus grand que nbr, alors il faut renvoyer x

stopGood:
        popl    %edx            # il faut sortir la dernière entrée de la pile car on saute le popl qui s'en charge normalement
        iaddl   1,%ecx          # on incrémente x car le calcul juste était celui de (x+1)²
