...
public class SpriteRobot {
    ...
    public void animateMove(Position target) {
        ...
        if (positionPath == null) {
            ...
            path.getElements().add(new MoveTo(robot.position.getX() * ImageResource.size + ImageResource.size / 2,
                    robot.position.getY() * ImageResource.size + ImageResource.size / 2));
            ...
            ptr.setDuration(Duration.millis(200 * robot.distance(target)));
            ptr.setPath(path);
            ptr.setNode(getImg());
