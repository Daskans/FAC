...
server.on("request", async (req, res) => {
    if (req.url.startsWith('/public/')) {
        ...
        try {
            const sqlQuery = "SELECT fichier FROM photos JOIN orientations ON photos.orientation=orientations.id WHERE orientations.nom='portrait'"; //Requête SQL
        ...
        const imgQuery = "SELECT photos.nom as titre, photographes.nom, prenom FROM photos JOIN photographes ON id_photographe=photographes.id WHERE photos.id="+imgNumber+";";
        const imgResult = await client.query(imgQuery);
        const titreImage = imgResult.rows.map(row => row.titre);
        const nomPhotographe = imgResult.rows.map(row => row.nom);
        const prenomPhotographe = imgResult.rows.map(row => row.prenom);
        console.log(imgResult.rows);
        let pageHTML = "<!DOCTYPE html><html><body>";
        pageHTML += '<h1>Image</h1>';
        pageHTML += '<h3>'+titreImage+'</h3>'
        pageHTML += '<img src="/public/image'+imgNumber+'.jpg" width="400" />';
        pageHTML += '<h3>'+nomPhotographe+" "+prenomPhotographe+'</h3>'
        pageHTML += '</body></html>';
        res.end(pageHTML);
    } else {
