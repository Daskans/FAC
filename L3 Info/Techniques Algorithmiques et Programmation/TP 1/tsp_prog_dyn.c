  ...
  */
  ...
  for (int column = 0; column < C; column++) {
    for (int row = 0; row < L; row++) {
      if (running) {
        if (column == 1 << row) {
          D[row][column].length = dist(V[n - 1], V[row]);
          D[row][column].pred = n - 1;
        } else if (column & 1 << row) {
          int min = -1;
          for (int x = 0; x < L; x++) {
            if (running && x != row && column & 1 << x) {
              int T = DeleteSet(column, row);
              int distance = D[x][T].length + dist(V[x], V[row]);
              if (min == -1 || distance < min) {
                min = distance;
                D[row][column].length = distance;
                D[row][column].pred = x;
                int k = ExtractPath(D, row, column, n, Q);
                drawPath(V, n, Q, k);
                if (!running) return 0;
              }
            }
