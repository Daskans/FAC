...
Print imp_trans. 
(* Affiche le terme de preuve du théorème *)

End Demo. (* fin de section *)
...
Check exemple.
(* Quand on ferme la section, les théorèmes démontrés sont 
   "exportés" avec comme prémisses supplémentaires, les 
   hypothèses de la section *)

End Demo'.

Check exemple.
(* Les "variables" utilisées, qui sont généralisées sous 
   forme quantifiée "pour tout" *)
...
Check douteux. (* "P->Q" *)
(* L'énoncé du théorème est néanmoins utilisable. *)
Print douteux.
(* En fait, "Admitted" est plutôt pour le preuves qu'on
   "fera plus tard"; pour renoncer vraiment à prouver un
   théorème (parce qu'il est faux peut-être), la bonne
   commande est "Abort" *)

Theorem tres_douteux: P.
Proof.
Abort.
...
(* le point-virgule enchaine les tactiques *)
(* Ici le "apply Hpqr" génère deux sous-buts (P, et Q),
   mais grâce au point-virgule, chaque sous-but est transmis 
   à la tactique "assumption" qui les résoud ; du coup,
   un seul "assumption" suffit *)
Qed.
...
intros p Hnp.
destruct (Hnp p).
(* Hnp a pour type P->False, donc (Hnp p) a pour type False;
