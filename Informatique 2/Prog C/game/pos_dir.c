...
void set_position(unsigned int x, unsigned int y, position_t *p_pos) {
    if (p_pos == NULL || p_pos->x==BORDER_CELL||p_pos->y==BORDER_CELL) {
        printf("error : invalide parameters");
        exit(EXIT_FAILURE);
    }
    else {
        p_pos->x = x;
...
void translate(position_t *p_position, direction_t *p_dir, position_t *p_newposition) {
    if (p_position == NULL || p_dir == NULL ||  
            p_position->x==BORDER_CELL || 
            p_position->y==BORDER_CELL || 
            p_position->x + p_dir->deltax < 0 || 
            p_position->y + p_dir->deltay < 0 || 
            p_position->x + p_dir->deltax >  p_position->x + 1 || 
            p_position->y + p_dir->deltay >  p_position->y + 1) {
